...
# pcolazurdo: this is extracted from ekylibre project @ https://github.com/ekylibre/ekylibre.git
...
module ApplicationHelper
  ...
  #     # label = "activerecord.attributes.#{model_name}.#{attribute.to_s}".t(default: default) # If you want internationalization you have to solve this with I18N
  #     label = "activerecord.attributes.#{model_name}.#{attribute.to_s}"
  ...
  #   if exporter.natures.any? and DocumentTemplate.of_nature(exporter.natures.map(&:name)).any?
  #
  ...
  def sort_params(sort)
    old_params = sort.to_s.split(',')
    #puts "1", old_params
    new_params = []
    old_params.split(",").each do |i|
      x = i.split(" ")
      if !x[1].nil?
        case x[1].downcase
        when "asc"
